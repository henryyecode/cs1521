	...
	if (PageTable[pageno].status.loaded == 1) {
	    ...
	    if (flag == 0) {
	        ...
	        for (int i = 0; i < nPages; i++) {
	            if (time > PageTable[i].lastAccessed && PageTable[i].status.loaded != 0) {
                    time = PageTable[i].lastAccessed;
                    oldestPage = i;
                    newFrame = PageTable[i].frameNo;
	            }     
	        }         
	        PageTable[oldestPage].status.loaded = 0;
	        nReplaces++;
	        if (PageTable[oldestPage].status.modified == 1)
	            nSaves++;
	        PageTable[oldestPage].status.modified = 0;
	        PageTable[oldestPage].lastAccessed = -1;
	        MemFrames[newFrame] = pageno; 
	        PageTable[oldestPage].frameNo = -1;     
	    }
	    ...
	    if (action == 'W')
	        PageTable[pageno].status.modified = 1;
	    PageTable[pageno].frameNo = newFrame;
	    PageTable[pageno].lastAccessed = clock;
	    pAddr = PageTable[pageno].frameNo * PAGESIZE + offset;
